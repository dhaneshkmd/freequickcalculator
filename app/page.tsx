...
function computeSIP({ monthly = 10000, rate = 12, years = 10 }:{monthly:number;rate:number;years:number;}) {
  ...
  const invested = monthly*n, gain = future - invested;
  return { future, invested, gain };
}
function SIPCalculator() {
  const [monthly,setMonthly]=useState(10000); const [rate,setRate]=useState(12);
  const [years,setYears]=useState(10); const [ccy,setCcy]=useState('INR');
  ...
  return (
    <CalculatorCard title="SIP Calculator" subtitle="Monthly investing growth projection">
      ...
      <MiniTable rows={[
        ['Monthly', currencyFmt(monthly, ccy)], ['Years', numberFmt(years)],
        ['Annual Return', `${rate}%`], ['Months', numberFmt(years*12)],
      ]}/>
...
function EMICalculator() {
  const [p,setP]=useState(500000), [rate,setRate]=useState(9), [years,setYears]=useState(5);
  const [ccy,setCcy]=useState('INR'); const res = useMemo(()=>computeEMI({principal:p,annualRate:rate,years}),[p,rate,years]);
  return (
    <CalculatorCard title="Loan EMI Calculator" subtitle="Monthly equated installment & totals">
      <FormGrid>
        <NumberField label="Loan Amount" value={p} onChange={setP} suffix={ccy}/>
...
function computeBMI({heightCm=170, weightKg=70}:{heightCm:number;weightKg:number;}) {
  ...
  let category = 'Normal'; if (bmi<18.5) category='Underweight'; else if (bmi<25) category='Normal';
  else if (bmi<30) category='Overweight'; else category='Obese'; return { bmi, category };
}
function BMICalculator() {
  const [h,setH]=useState(170), [w,setW]=useState(70);
  const res = useMemo(()=>computeBMI({heightCm:h,weightKg:w}),[h,w]);
  return (
    <CalculatorCard title="BMI Calculator" subtitle="Body Mass Index (WHO ranges)">
      <FormGrid>
        <NumberField label="Height" value={h} onChange={setH} suffix="cm"/>
        <NumberField label="Weight" value={w} onChange={setW} suffix="kg"/>
      </FormGrid>
...
function CalculatorCard({ title, subtitle, children }:{title:string;subtitle?:string;children:React.ReactNode;}) {
  return (
    <div className="rounded-2xl p-5 md:p-6 shadow-lg bg-white/80 dark:bg-slate-900/80 border border-slate-200/60 dark:border-slate-800">
      <div className="mb-4"><h2 className="text-xl md:text-2xl font-semibold">{title}</h2>
      {subtitle && <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{subtitle}</p>}</div>
      {children}
    </div>
  );
}
...
export default function Page() {
  ...
  const [query,setQuery]=useState(''); const [active,setActive]=useState<string|null>(null);
...
if (typeof window!=='undefined') {
  window.addEventListener('keydown',(e)=>{
    ...
    if (e.key==='/' && !(t && (t.closest('input')||t.closest('textarea')))) {
      e.preventDefault();
