...
# Clean repo (keep .git)
rm -rf app components data lib public styles node_modules package-lock.json .next \
       tailwind.config.* postcss.config.* next.config.* tsconfig.json next-env.d.ts \
...
export function emiSchedule(P:number, apr:number, months:number){
  ...
  for(let m=1;m<=Math.min(12,n);m++){
    const interest=i===0?0:bal*i; const principal=Math.min(emi-interest, bal);
